# Mohammad Ashiqur Rahman

### Full-Stack Engineer | Building Scalable Platforms

I'm a Full-Stack Developer at [AFQ I Service gGmbh](https://amuvee.de), leading the development of scalable platforms from frontend to backend. With a Master's in Automotive Software Engineering and expertise across the full stack, I architect and optimize digital solutions that serve thousands of users.

---

## 🚀 Core Expertise

**Frontend:** Angular • React • Next.js • Redux • TypeScript • TailwindCSS

**Backend:** Node.js • NestJS • Express.js • TypeScript

**Database:** MongoDB • PostgreSQL • MySQL • Redis

**DevOps & Cloud:** Docker • AWS • Google Cloud • CI/CD

**Currently Exploring:** Microfrontend Architecture • AI Integration • Advanced DevOps

---

## 🔭 What I'm Doing

- Leading full-stack development and optimization of scalable platforms at AFQ I Service gGmbh
- Architecting solutions from frontend to backend for optimal performance and user experience
- Exploring Microfrontend architecture and cloud-native patterns
- Contributing to open-source and mentoring developers

---

## 🤝 Let's Connect

Open to collaborating on innovative projects and discussing modern web architecture!

**Reach me:** [LinkedIn](https://www.linkedin.com/in/ashiq-dev/) • [rahman.tuc@gmail.com](mailto:rahman.tuc@gmail.com)

